...
import { TransactionBorrow } from '@/classes/transaction/TransactionBorrow';
...
const TRANSACTION_VALIDATION_STATUSES = [
  ...
  ['Borrow is paused', false],
  ['Token is currently archived', false],
  ['Borrowing limit reached', false],
  ['Not enough tokens available on the market', false],
  ['Insufficient collateral', false],
  ['Global token limit is exceeded, please try again tomorrow', false],
  ['Market borrow cap reached', false],
  ['Borrow', true],
] as const;

describe('Test TransactionBorrow', () => {
  it('should create generator on exec new instance', () => {
    expect(mockedCreateTransactionExecutorGenerator).toBeCalledTimes(0);
    void new TransactionBorrow(mockedMarket);
  ...
  it.each(TRANSACTION_INPUT_VALUES)(
    ...
    (input_value, input_result) => {
      const transaction = new TransactionBorrow(mockedMarket);
